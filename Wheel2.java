...
public class Wheel2 {
   ...
   private void loadRToL2() {
      for (int i = 0; i < nRec; i++) {
         rToL.set(lToR.get(i), i);
      }
   ...
   // nPos = notch position
   //
   public Wheel2(int nEnts, long seedIn, int cPos, int nPos) {
      if (nEnts > MAXSIZE || nEnts <= 0) {
         System.out.println("Size of the wheel must be a positive number less than or equal to" + MAXSIZE);
         System.exit(nEnts);
      }
      if (nPos < 0 || nPos >= nEnts) {
         System.out.println(
               "The notch position should be greater or equal to 0 and less than size of the wheel (" + MAXSIZE + ")");
         System.exit(nPos);
      }
      if (cPos < 0 || cPos >= nEnts) {
         System.out.println("The current position should be greater or equal to 0 and less than size of the wheel ("
               + MAXSIZE + ")");
         System.exit(cPos);
      }
      nRec = nEnts;
